...
class NotificationNavigationHandler {
  ...
  void init() {
    _subscription = _notificationService.onNotification.listen(
      _handleNotification,
    );
  ...
  void _handleNotification(BarzNotification notification) {
    debugPrint(
      '[NotificationHandler] Handling notification: ${notification.type}',
    );
    ...
    if (notification.type == NotificationType.orderUpdate ||
      ...
      if (orderId != null) {
        ...
        appRouter.push(
          AppRoute.order.path.replaceFirst(':orderId', orderId.toString()),
        );
      }
    }
  }
