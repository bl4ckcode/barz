...
class PaymentNetworkDatasource implements PaymentDatasource {
  ...
  Future<List<PaymentMethod>> getPaymentMethods() async {
    try {
      final response = await dio.get('${ApiEndpoints.baseUrl}${ApiEndpoints.paymentMethods}');
      return (response.data as List).map((json) => PaymentMethod.fromJson(json)).toList();
    } on DioException catch (e) {
      throw ServerException(e.response?.data?['detail'] ?? 'Failed to get payment methods', e.response?.statusCode);
    }
  }

  @override
  Future<PaymentMethod> addPaymentMethod(PaymentMethod method, String? cardToken) async {
    try {
      final data = method.toJson();
      if (cardToken != null) data['card_token'] = cardToken;
      final response = await dio.post('${ApiEndpoints.baseUrl}${ApiEndpoints.paymentMethods}', data: data);
      return PaymentMethod.fromJson(response.data);
    } on DioException catch (e) {
      throw ServerException(e.response?.data?['detail'] ?? 'Failed to add payment method', e.response?.statusCode);
    }
  }
  ...
  Future<PaymentMethod> setDefaultPaymentMethod(int methodId) async {
    try {
      final response = await dio.put('${ApiEndpoints.baseUrl}${ApiEndpoints.paymentMethods}/$methodId/default');
      return PaymentMethod.fromJson(response.data);
    } on DioException catch (e) {
      throw ServerException(e.response?.data?['detail'] ?? 'Failed to set default payment method', e.response?.statusCode);
    }
  }
  ...
  Future<bool> removePaymentMethod(int methodId) async {
    try {
      await dio.delete('${ApiEndpoints.baseUrl}${ApiEndpoints.paymentMethods}/$methodId');
      return true;
    } on DioException catch (e) {
      throw ServerException(e.response?.data?['detail'] ?? 'Failed to remove payment method', e.response?.statusCode);
    }
  }
  ...
  Future<Transaction> checkPaymentStatus(int transactionId) async {
    try {
      final response = await dio.get('${ApiEndpoints.baseUrl}${ApiEndpoints.payments}/$transactionId/status');
      return Transaction.fromJson(response.data);
    } on DioException catch (e) {
      throw ServerException(e.response?.data?['detail'] ?? 'Failed to check payment status', e.response?.statusCode);
    }
  }

  @override
  Future<List<Transaction>> getTransactionHistory({int? limit, int? offset}) async {
    try {
      final queryParams = <String, dynamic>{};
      if (limit != null) queryParams['limit'] = limit;
      if (offset != null) queryParams['offset'] = offset;
      final response = await dio.get('${ApiEndpoints.baseUrl}${ApiEndpoints.transactions}', queryParameters: queryParams.isNotEmpty ? queryParams : null);
      return (response.data as List).map((json) => Transaction.fromJson(json)).toList();
    } on DioException catch (e) {
      throw ServerException(e.response?.data?['detail'] ?? 'Failed to get transaction history', e.response?.statusCode);
    }
  }
  ...
  Future<Transaction> topUpWallet(double amount, PaymentType paymentType, {int? paymentMethodId}) async {
    try {
      ...
      throw ServerException(e.response?.data?['detail'] ?? 'Failed to top up wallet', e.response?.statusCode);
    }
  }
}
