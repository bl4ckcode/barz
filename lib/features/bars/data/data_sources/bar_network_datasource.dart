...
class BarNetworkDataSource {
  ...
  Future<List<BarModel>> getNearbyBars(
    ...
    try {
      ...
      return (response.data as List).map((e) => BarModel.fromJson(e)).toList();
  ...
  Future<BarModel> getBar(int barId) async {
    try {
      final response = await dio.get(
        '${ApiEndpoints.baseUrl}${ApiEndpoints.bar(barId)}',
      );
  ...
  Future<RecentOrdersResponse> getBarOrders(
    int barId, {
    int limit = 10,
    String status = 'all',
    int page = 1,
    ...
    try {
      final response = await dio.get(
        ...
        queryParameters: {'limit': limit, 'status': status, 'page': page},
      );
  ...
  Future<BarStatus> toggleBarStatus(
    int barId,
    bool isOpen, {
    String? reason,
  }) async {
  ...
  Future<void> deleteMenuItem(int menuId, int itemId) async {
    try {
      await dio.delete('${ApiEndpoints.baseUrl}/menus/$menuId/items/$itemId');
    } on DioException catch (e) {
      throw ServerException(
  ...
  Future<void> deleteMenu(int menuId) async {
    try {
      await dio.delete('${ApiEndpoints.baseUrl}/menus/$menuId');
    } on DioException catch (e) {
      throw ServerException(
  ...
  Future<LocationConfig> getLocationConfig(int barId) async {
    try {
      final response = await dio.get(
        '${ApiEndpoints.baseUrl}${ApiEndpoints.barLocationConfig(barId)}',
      );
      return LocationConfig.fromJson(response.data);
    } on DioException catch (e) {
      // If 404 or verify, fallback to defaults?
      // Assuming backend implements it.
      throw ServerException(
        e.response?.data?['detail'] ?? 'Failed to fetch location config',
        e.response?.statusCode,
      );
    }
  }

  Future<List<Promotion>> getActivePromotions(int barId) async {
    try {
      final response = await dio.get(
        '${ApiEndpoints.baseUrl}${ApiEndpoints.barActivePromotions(barId)}',
      );
      return (response.data as List).map((e) => Promotion.fromJson(e)).toList();
    } on DioException catch (e) {
      throw ServerException(
        e.response?.data?['detail'] ?? 'Failed to fetch promotions',
        e.response?.statusCode,
      );
    }
  }
